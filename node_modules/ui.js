    ...
    var _renderFnList = {
      'build': function (html, data) {
        ...
        data.map(function (o, i) {
          tpl += '<li><a class="item" data-index="' + i + '" href="#" ><span class="action select-build" action="select-build">' + o.name + '</span><i class="modify action" action="modify-build">修改</i><i class="del action" action="del-build">删除</i></a></li>'
        })
        ...
        function _delBuild (btn) {
          btn = $(btn)
          var index = btn.parent().data('index'),
            p = btn.parents('li')
        ...
        function _saveBuild () {
          ...
          if (itemObj) {
            ...
            list.find('li').last().before('<li><a class="item" data-index="' + idx + '" href="#" ><span class="action select-build" action="select-build">' + name + '</span><i class="modify action" action="modify-build">修改</i><i class="del action" action="del-build">删除</i></a></li>')
            listScroller.update()
            
            _selectBuild($('.build-box .item[data-index="' + idx + '"]').find('.select-build'))
          }
          _saveToFile(data)
          _resetForm()
        }

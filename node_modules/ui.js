...
var tplList = {
  ...
  'build': '<div class="build-box">\
              ...
              <div class="console">\
                ...
                <button class="action clear" action="clear">清除</button>\
...
var UI = (function () {
  ...
  function initPanel () {
    ...
    var _renderFnList = {
      'build': function (html, data) {
        ...
        function _runBuild () {
          ...
          id++
          _output('<p class="notification new-task">[' + new Date().toLocaleString() + '] 任务号：<em class="no">' + id + '</em>，执行：' + itemObj.name + '</p>')
    ...
    function _init (channel) {
      var html = '<div class="content-panel ' + channel + '-panel">', channelObj = null
      
      //lazy load
      if (!tplList[channel]) {
        channelObj = require('panel/' + channel)
        tplList[channel] = channelObj.tpl
        _renderFnList[channel] = channelObj.render
      }
      
      html += tplList[channel]
      html += '</div>'
      channelInfo[channel] = data[channel] || true

...
var UI = (function () {
  ...
  function initPanel () {
      ...
      'build': function (html, data) {
        ...
        function _saveBuild () {
          ...
          if (itemObj) {
            ...
            list.find('.current .select-build').text(name)
          } else {
            data.push({
              name: name,
        ...
        function _up () {
          var items = $('.build-box .item'),
            item = items.filter('.current'),
            index = items.index(item) || 0
          items.eq(index - 1).parent().find('.action').eq(0).trigger('click')
        }
        ...
        function _down () {
          var items = $('.build-box .item'),
            ...
            item = items.filter('.current'),
            index = items.index(item) || 0
          items.eq((index + 1) % l).parent().find('.action').eq(0).trigger('click')
        }
    ...
    ipc.on('up', function () {
      panel[currentChannel].up && panel[currentChannel].up()
    })
    
    ipc.on('down', function () {
      panel[currentChannel].down && panel[currentChannel].down()
    })
  }
